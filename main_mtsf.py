...
def main(net_init, config, data, data_name, MODEL, model_name, file_res):
    ...
    file_res.write('{}, {}, {}, {}, {}\n'.format(config.horizon, net_init.RNNUnits, net_init.CNNFilters, test_rse, test_corr))

    del X_train, y_train, X_valid, y_valid, X_test, y_test, model, optimizer
    gc.collect()
...
if __name__ == '__main__':
    ...
    for data_name, config in zip(Data_Config.keys(), Data_Config.values()):
        for model_name, model in zip(Model.keys(), Model.values()):
            ...
            file_res.write('horizon, rnn-units, cnn-units, RSE, CORR\n')
